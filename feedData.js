...
/*Reasoning the multi-valued parameters*/
	var behaviour = 0;
    ...
    var habitat = 0;
	for (var i=0;i<req.body.Habitat.length;i++)
		habitat += parseInt(req.body.Habitat[i], 10)
	...
	con.query(query_stmt, function (err, result, fields) 
	{
		...
		if (result.length == 0)//entry is absent
		{
				...
				insert_stmt = "insert into X_birds (CommonName, ScientificName, Classification, Behaviour, Plumage, Size, Habitat) values ('"+escaped_name+"','"+sciName+"','"+classification+"',"+behaviour+","+plumage+","+size+",'"+habitat+"')"; 
